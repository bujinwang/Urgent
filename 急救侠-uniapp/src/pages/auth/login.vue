<template>
  <view>
    ...
    <view style="margin:0 40rpx;background:#fff;border-radius:24rpx;padding:30rpx">
      ...
      <input style="width:100%;height:44px;border:1px solid #ddd;border-radius:8px;padding:0 12px;font-size:16px;margin-bottom:12px;box-sizing:border-box" v-model="pwd" placeholder="密码" />
...
async function submit(){
  ...
  try{
    const result=mode.value==='register'?await phoneRegister(phone.value,pwd.value,name.value):await phoneLogin(phone.value,pwd.value)
