...
> ⚠️ **`--at` 的解析是「宽松」的，注意两个易混写法**（实现用 `Date.parse`）：
> - `2026-09-12T20:00:00Z`（**带 `Z`/偏移**）→ 按该偏移解析，**推荐这种写法**；
> - `2026-09-12`（**裸日期**）→ 按 **UTC 零点**解析；
> - `2026-09-12T20:00:00`（**无偏移的日期时间**）→ 按**本机时区**解析。
>
> 三者混用会得到不同的绝对时刻。好在命令**会把解析结果同时按 UTC 与本地回显**（`即 …Z（UTC） / …（本地）`）——**请回显核对一眼**，确认与你实际发送的时刻一致再收工。
