...
rescueRouter.get('/certifications', (req, res) => {
  try {
    const rows = all('SELECT * FROM external_certifications WHERE user_id=? ORDER BY created_at DESC', req.query.userId as string)
...
rescueRouter.post('/mobilize', (req, res) => {
  try {
    ...
    const leader = get('SELECT is_leader FROM users WHERE id=?', leaderId)
...
rescueRouter.get('/mobilizations', (_req, res) => {
  try {
    const rows = all('SELECT * FROM emergency_mobilizations ORDER BY created_at DESC LIMIT 20', )
...
rescueRouter.put('/mobilizations/:id/approve', (req, res) => {
  try {
    ...
    const mob = get('SELECT leader_id FROM emergency_mobilizations WHERE id=?', req.params.id)
...
rescueRouter.get('/mobilizations/:id/volunteers', (req, res) => {
  try {
    const rows = all('SELECT * FROM mobilization_volunteers WHERE mobilization_id=?', req.params.id)
...
rescueRouter.get('/team', (req, res) => {
  try {
    const leader = get('SELECT affiliation FROM users WHERE id=? AND is_leader=1', req.query.leaderId as string)
    if (!leader) return res.json(error('非认证领导者'))
    const rows = all("SELECT u.id, u.name, u.avatar, u.tier, u.rescue_count, u.city FROM users u WHERE u.affiliation=? ORDER BY u.rescue_count DESC", leader.affiliation)
...
rescueRouter.get('/mobilizations/:taskId/media', (req, res) => {
  try {
    const rows = all('SELECT * FROM task_media WHERE task_id = ? ORDER BY created_at DESC LIMIT 50', req.params.taskId)
...
rescueRouter.get('/live/:taskId', (req, res) => {
  try {
    const rows = all("SELECT * FROM live_sessions WHERE task_id=? AND ended_at IS NULL ORDER BY started_at DESC", req.params.taskId)
...
rescueRouter.post('/live/end/:sessionId', (req, res) => {
  try {
    ...
    const s = get('SELECT * FROM live_sessions WHERE id=?', req.params.sessionId)
