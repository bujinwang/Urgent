...
drillRouter.get('/events', (_req, res) => {
  try {
    const rows = all('SELECT * FROM drill_events ORDER BY date ASC LIMIT 20', )
...
drillRouter.put('/events/:id/complete', (req, res) => {
  try {
    const ev = get('SELECT * FROM drill_events WHERE id=?', req.params.id)
    ...
    const parts = all('SELECT user_id FROM drill_participants WHERE event_id=?', req.params.id)
...
drillRouter.get('/organizers', (_req, res) => {
  try {
    const rows = all("SELECT u.id,u.name,u.avatar,u.tier,u.points,(SELECT COUNT(*) FROM drill_events WHERE organizer_id=u.id AND status='completed') as drills_completed FROM users u WHERE u.is_organizer=1 ORDER BY drills_completed DESC", )
