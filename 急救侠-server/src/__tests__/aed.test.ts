...
describe('AED Routes', () => {
  ...
  describe('GET /api/aed/nearby', () => {
    ...
    it('accepts lat/lng query params', async () => {
      const res = await request(server).get('/api/aed/nearby?lat=22.5&lng=113.9')
      expect(res.status).toBe(200)
  ...
  describe('GET /api/aed/:id', () => {
    it('returns AED by id', async () => {
      const res = await request(server).get('/api/aed/aed_001')
      expect(res.status).toBe(200)
      expect(res.body.data.id).toBe('aed_001')
    })

    it('returns error for unknown id', async () => {
      const res = await request(server).get('/api/aed/nonexistent')
