...
describe('Edge Cases', () => {
  ...
  describe('AED edge cases', () => {
    it('nearby returns empty when no AEDs', async () => {
      clearAll()
      const res = await request(server).get('/api/aed/nearby')
      expect(res.body.data).toEqual([])
    })

    it('detail returns error for non-existent', async () => {
      const res = await request(server).get('/api/aed/zzz')
      expect(res.body.code).toBe(-1)
    })
  })
  ...
  describe('News edge cases', () => {
    ...
    it('category returns empty when no match', async () => {
      clearAll()
      const res = await request(server).get('/api/news/category/nonexistent')
      expect(res.body.data).toEqual([])
    })
  ...
  describe('Atlas edge cases', () => {
    it('cards returns empty when none', async () => {
      clearAll()
      const res = await request(server).get('/api/atlas/cards')
      expect(res.body.data).toEqual([])
    })
  })
  ...
  describe('Media Alert edge cases', () => {
    ...
    it('status for any uploadId works', async () => {
      const res = await request(server).get('/api/media-alert/status/random_id')
      expect(res.status).toBe(200)
      expect(res.body.data.status).toBe('success')
    })
  })
