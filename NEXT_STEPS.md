...
- 剩余：`AED 短信/电话降级` —— **短信降级已完成**（见下节）；**语音降级仍待做**（需 SLA 超时调度或状态报告回调）。
...
- **待办**：① 模板变量现用 `{device, address}`，**报备通过后按真实模板变量名对齐**（只改一处）；② **语音降级**（收不到短信的老人机）需 SLA 超时调度或状态报告回调（0.11 元/分钟 ≈ 短信 2.4 倍）；③ 阿里云企业实名 + 签名/模板报备（1–2 工作日）。
