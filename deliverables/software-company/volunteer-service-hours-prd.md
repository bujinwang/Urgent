...
> 版本：v1.1 ｜ 状态：**✅ Q1/Q2/D8 已决（2026-09-17，业务方）⇒ 可开工**；Q3（归因方案，架构师定）、Q3b、Q4~Q9 待定 ｜ 语言：中文
...
**而"归因"之外还缺"闭合"** —— 即使补上"谁参与了"，`duration_min` 仍算不出来，因为**现有参与记录一律只有"开始"没有"结束"**。

**★ 三个场景其实是「三档完成度」，不是有/无的二元**（这张表决定 P0-2 的设计形态）：

| 档 | 场景 | 人×事件表 | 报名时刻 | 出席/完成标记 | 闭合时刻 |
|---|---|---|---|---|---|
| **③ 最高** | **演习** `drill` | ✅ `drill_participants`（`db.ts:309-319`） | ✅ `joined_at` | ✅ **`attended` 0→1 回写**（`drill.ts:46`） | ❌ **无 `ended_at`** |
| **② 中** | **应急动员** `mobilization` | ✅ `mobilization_volunteers`（`db.ts:399-409`） | ✅ `responded_at` | ⚠️ **有 `status` 列但无任何状态流转** —— 全仓**无 `UPDATE mobilization_volunteers`** ⇒ `status` 恒为 `'responded'`，**无法区分报名与出席** | ❌ **无 `ended_at`** |
| **① 最低** | **救援任务** `tasks` | ❌ **连表都没有** | ❌ 只有 `volunteers_responded` 计数器 | ❌ | ❌ |

⇒ **`drill_participants` 是三者中最接近目标形态的**，因为它已经区分「报名（`joined_at`）」与「**出席（`attended`）**」——
而**时长只应算出席，不应算报名**。这个语义正是 P0-2 要的，且**已有实现可参照**（`db.ts:314` 的 `attended INTEGER NOT NULL DEFAULT 0` + `drill.ts:46` 的回写）。
`drill.ts:46-49` 在回写 `attended=1` 时，还同步发积分并写入 `training_records` ⇒ 该端点**已经是"完成即结算"的现成范式**。

**★ 好消息：幂等写法已有现成先例可直接照抄，不必新发明**

`mobilization_volunteers` 写入点 `rescue.ts:69-73` 已经用了
**`UNIQUE(mobilization_id, user_id)` 约束 + `INSERT OR IGNORE`** 来做幂等 ——
与 P0-2 要求的"同一用户 accept 两次仍只有一条"**是同一个模式**。
`drill_participants`（`db.ts:309-319`，`UNIQUE(event_id, user_id)`，写入点 `drill.ts:30` 的 `INSERT OR IGNORE`）**同理**。

⇒ 结论不变但更精确：**F4 缺的不是"幂等技巧"、也不是"出席语义"（演习侧已有），而是**
**(a) 任务侧连归因表都没有、(b) 三条链路全都缺闭合时刻、(c) 三条链路口径不一，需要归一。**
...
写入点 `drill.ts:30`（`INSERT OR IGNORE`，= **报名**）；`attended` 在 `drill.ts:46` 由 `/events/:id/complete` **回写为 1**（= **出席**）；
并在 `drill.ts:48-49` **自动为每位参与者补一条 `training_records`**、`drill.ts:45` 每人 +20 积分。
⇒ **演习侧"归因 + 出席语义"都齐了**（三档中最高档，见上表），**唯一缺的仍是"闭合时刻"**
（`drill_participants` 无 `ended_at`，且 `drill_events` 只有 `date TEXT` 无起止时刻）。
...
> **本 PRD 明确声明：我不能确认该标准的具体口径（字段格式、认定规则、认定主体、是否需实名）。**
> **Q1 已决（2026-09-17，业务方）：先做自有口径、暂不宣称合规** ⇒ 交付物**一律不得**标注"符合国家…标准"或"官方"，
> 该点已升级为**交付验收项**（见 §10、§11 Q1、测试 T19）。**P2-11 保持待定**。
...
| **D8** | **证明的措辞** | ✅ **已确认（2026-09-17，业务方）**：UI 与导出物统一用「**志愿服务记录证明（急救侠平台出具）**」，**不使用**"官方""国家标准""政府认可"字样 | 避免重复 §1.1 缺口 6 那种"看着像、实际不可验"的失实陈述；"官方"需政府背书（建议书 §06 政府方职责含"荣誉证书、政府表彰"）。**Q1 已决（见 §11）后本条升级为强制口径**：UI / CSV / PDF / 文档一律不得出现"符合国家标准""国标""官方" |
...
- ✅ **幂等写法与"出席语义"都有现成先例可照抄**：
  - 幂等 → `mobilization_volunteers` 的 **`UNIQUE(...)` 约束 + `INSERT OR IGNORE`**（`db.ts:399-409` + `rescue.ts:69-73`），或 `drill_participants` 同款（`db.ts:309-319` + `drill.ts:30`）；
  - **出席语义 → `drill_participants.attended`**（`db.ts:314`，`DEFAULT 0`，由 `drill.ts:46` 回写为 1）。⚠️ **时长只应算"出席"，不应算"报名"** —— 因此新表**必须**有等价的出席/完成标记，**不可只靠"有行存在"判定**；
  - **"完成即结算"范式 → `drill.ts:46-49`**（回写 `attended` + 发积分 + 写 `training_records` 在同一处收口），新端点可照此形态。
- 验收：① 同一用户对同一任务 accept 两次 ⇒ 仍**只有一条**参与记录；② **报名但未出席 ⇒ 该条不产生时长**（等价于 `drill` 里 `attended=0`）；③ `complete` 后该条**必须有** `ended_at_ms`（缺失即视为未闭合，**不计入时长**）；④ `/complete` 被重复调用 ⇒ 幂等，**不产生第二条**、不重复累加时长。
...
- ✅ **Q2 已决（2026-09-17，业务方）：不实名，用自填昵称** ⇒ 新增两条验收：
  - ⑤ **证明上的姓名取自 `users.name`（自填昵称）**，系统**不新增**姓名/证件号字段、**不校验**昵称与实名的一致性；
  - ⑥ **UI 与导出物必须显著明示「可采信度有限（平台出具，非实名认证）」** ⇒ 该免责文案是**交付物的一部分**，其缺失必须能被测试咬住（见 T19）。
- 验收：① 两次对同一区间生成 ⇒ 编号**不同**、总时长**相同**；② 作废后按编号查询返回 `revoked`；③ 验真接口**不返回** `user_id` / `name` / 任何 PII；④ 总时长只含 `is_drill=0 AND status='confirmed' AND ended_at_ms IS NOT NULL`；⑤⑥ 见上。
...
**P2-11 · 国家标准口径对齐改造** —— 待 Q1 有权威口径后，可能涉及字段更名/新增、认定规则、计时长短规则。**极可能是破坏性变更**，这正是它必须等权威口径的原因。
> ⚠️ **强制约束**：**Q1 有权威口径前，任何对外物（UI / CSV / PDF / 文档 / 建议书）不得出现合规宣称。**
> Q1 已决为"先做自有口径、暂不宣称合规"（§11），本条因此**保持待定**，不因 Q1 已决而开工。
**P2-12 · PDF 版式证明 / 政府联合署名 / 二维码验真** —— 需真实二维码（替代 `cert/index.vue:22` 的 CSS 假码）与政府背书授权。

### ⚠️ 连带待办（**两条合并为一条，不要开两条**）

| # | 事项 | 说明 |
|---|---|---|
| **B-1** | **建议书 §10 措辞与实现对齐** | `急救侠_项目建议书_广州海南_v1.html` §10 原文含两处与新实现/新口径不一致的表述：<br>① **F4 相关**：「**符合国家志愿服务时长记录标准**」「志愿者可获得**官方志愿服务证明**」→ 需改为平台自有口径（D8 + Q1）；<br>② **F3 相关（既有遗留）**：「可追溯至账号**实名信息**」（实现为匿名可触发，见 `sos-telemetry-prd.md` D2）与「用户位置数据…**任务结束后自动删除**」（实现为不采位置，见 `sos-telemetry-design.md` §5.3）。<br>⇒ 两者**同属 §10 的对外措辞修订**，**合并为同一条待办**（F3 侧已在 `product-backlog.md:186-190` 挂起，本条与之合并）。<br>⚠️ **属对外提交文件，需业务侧定稿后再动。** |
...
**这是产品/合规决策，下表是我的建议方案；标 ✅ 的已被业务方拍板（2026-09-17），其余见 Q5/Q6。**

| 问题 | 口径 | 状态 |
|---|---|---|
| **时长从哪些事件算** | 仅 5 类：`rescue_task` / `drill` / `training` / `aed_checkin` / `manual` | 建议，待 Q5 确认 |
| **采什么** | 人、活动类型、起止时刻、时长、演习标记、机构归属 | 建议，待 Q5 确认 |
| **是否新增姓名 / 证件号** | ❌ **不新增**。证明姓名取自 `users.name`（自填昵称），**不采证件号、不做实名核验** | ✅ **已决（2026-09-17，业务方，对应 Q2）** |
| **不采位置轨迹 / 不采"在线时长"** | `volunteer_locations`（`db.ts:411-419`）的 `lat/lng` 心跳**不得**作为时长来源 | 建议，待 Q5 确认 |
| **保留多久** | 台账**长期保留**；证明**永久保留** | 建议，待 Q5 确认 |
| **谁能看** | 见 §5.4 | 建议，待 Q5 确认 |
| **可导出** | 本人随时导出自己的；机构可导出本机构成员 | 建议，待 Q5 确认 |
| **可删除** | **支持"作废"而非"物理删除"**：作废留 `reason` 与时间，验真接口返回 `revoked` | 建议，待 Q6 确认 |
...
| T19 | **措辞合规（Q1+Q2 双重强制项）**：证明 UI / CSV 导出文本 / 相关文档中，**不含**「符合国家标准」「国标」「官方」；且**含**「可采信度有限（平台出具，非实名认证）」免责文案 | 文案里加回「官方」⇒ 变红；删掉免责句 ⇒ 变红（**建议用深扫断言**，照 `role-split.test.ts` 的写法） |
| T20 | **`attended` 语义**：报名但未出席（等价 `attended=0`）的参与 ⇒ **不产生时长**，也不进证明 | 改成"有行即计时" ⇒ 变红 |
...
- 🚫 **不宣称**：**本轮交付不宣称符合任何国家标准，不得用于向第三方主张官方认定**（Q1 已决，2026-09-17）。
  ⇒ 这是**交付验收项**：UI / CSV / PDF / 文档出现「符合国家标准」「国标」「官方」即为**不通过**（见 T19）。
- ❌ **不做**：实名认证 / 证件号采集（**Q2 已决：不实名**，见 §6 已决项与 §11 Q2）。
...
### ✅ Q1（P0 级，**已决 2026-09-17 · 业务方**）「符合国家志愿服务时长记录标准」的口径

**决定：先做自有口径，暂不宣称合规。**

- 台账与证明**照常实现**（P0 全量 + P1）。
- **强制口径（交付验收项）**：**UI、CSV、PDF、文档一律不得出现「符合国家标准」「国标」「官方」字样。**
- 我此前的声明仍然有效并已写入交付物：**我无法确定该标准的具体口径**（字段格式 / 认定主体 / 是否需实名）——
  因此本 PRD 与实现**不得**以任何形式暗示已合规。
- **P2-11（国标口径对齐改造）保持待定**，见 §4 P2-11 的标注。

### ✅ Q2（P0 级，**已决 2026-09-17 · 业务方**）「官方志愿服务证明」是否必须实名

**决定：不实名，用自填昵称。**

- 证明姓名取自 `users.name`（自填昵称，用户可自行修改）；**不新增姓名/证件号字段、不做实名认证**（见 §6 已决项）。
- **连带强制项**：UI 与导出物**必须显著明示**「可采信度有限（平台出具，非实名认证）」—— 见 §4 P0-4 验收 ⑤⑥。
- 我此前指出的风险依然成立且已被业务方接受：**昵称自填 ⇒ 证明不复核主体身份**，因此措辞与免责声明**必须**如实反映这一点（D8 + Q1 双重要求）。
...
**A.2 从 `req.query` 取身份（**6 处** = 5 × `userId` + 1 × `leaderId`）**
...
| `rescue.ts` | 88 | `req.query.leaderId`（**唯一非 `userId` 的一处**） |

> **计数口径（防再次分歧）**：只 grep `req\.query\.userId` ⇒ **5 处**；把 `leaderId` 也计入"身份来源" ⇒ **6 处**。
> 本 PRD 采用后者（`rescue.ts:88` 用 `leaderId` 判定"是否认证领导"并据其结果返回队伍花名册，**属于身份判定**）。
> 结论：**body 26 处 + query 6 处 = 32 处**身份取自客户端。
