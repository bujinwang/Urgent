...
> 版本：**v1.6** ｜ 状态：设计定稿（§10 八条已全决；§11 v1.2–v1.4 修正已拍板；v1.5/v1.6 澄清口径），待实现 ｜ 语言：中文
...
- #6 **★ 归属口径 = (a) 按成员**（v1.6，§4.5-①）：**同一人的时长会在其每个所属机构的报表里各出现一次**（视图重复，非重复计）；**刻意不按 `l.org_id`**（该列恒空，见 §4.5-②）。T41 守"跨机构各自可见"。
- #7 **★ gov 全局口径**（v1.6，§4.5-③）：`participantCount = COUNT(DISTINCT user_id)`（**去重**）、`totalMinutes = SUM(duration_min)` 直接对台账（**只算一次**，不经机构相加）。T41 守"不翻倍"。
- #7 **★ gov `null` 判据 = "有无台账行"**（v1.6，§4.5-④）：**有行 ⇒ 返回对象（`totalMinutes` 可为 0）**，**无行 ⇒ `null`**；与 `/me` 口径统一。T42 守。
...
### 4.5 ★★ 聚合口径（v1.6 —— 澄清三处设计空白）

> 触发：T05 独立验证发现 —— 设计**未定义**"机构汇总按什么归属"，实现只能退化为"按成员 JOIN"；根因在**数据层**（见 ②）。

#### 4.5-① 机构汇总归属：**口径 (a) 按成员**（并明写"跨机构会重复"）

**选 (a)**，理由：**(b) 现在物理上不可行** —— 见 ②。

| | 口径 | 语义 + 必须写清 |
|---|---|---|
| **(a) 按成员** ✅ | 汇总 = "**该机构成员的（全部）服务**" | ★ **同一人的时长会在其所属的每个机构报表里各出现一次** —— 这是**机构视角的独立视图**、**不是重复计**（每份都自洽）。⚠️ **但跨机构相加会重复**（政府侧若要"不重复"须另设计，见 ③-②） |
| (b) 按台账 `org_id` | 汇总 = "**归属本机构的服务**" | **须先解决"`org_id` 从不写入"（②）** ⇒ **P1 / 另立**；且"服务归属于哪个机构"本身是**产品决策**（按事件发起方？按人当前机构？） |

**实现形状（(a)）**：`FROM volunteer_service_logs l JOIN organization_members om ON om.user_id = l.user_id WHERE om.org_id = ? AND COUNTING_WHERE` —— **刻意不用 `l.org_id`**（它恒为空串，用了必得空集）。**此点须在代码注释写明**，否则后人会以为"漏用了 org_id"。

#### 4.5-② ★ 根因（数据层）：`volunteer_service_logs.org_id` **从未被写入**

- T01 的 `closeServiceForUser()` → `recordService()` **从不传 `orgId`** ⇒ `recordService` 落到默认 `''` ⇒ **该列恒为空串**。
- ⇒ "按 `l.org_id` 过滤"**必然返回空集**；实现方的"按成员 JOIN"是**当前唯一能返回数据的办法**。
- ⇒ 结论：**(a) 不是"次优解"，是当前唯一可行解**；把 `org_id` 写起来（+ 定义归属规则）属 **P1 / 另立**，并登记为技术债（§9）。

#### 4.5-③ 政府看板（`gov`）**全局**口径：人次去重、总时长只算一次

| 指标 | 口径 | 说明 |
|---|---|---|
| **人次** `participantCount` | `COUNT(DISTINCT user_id)` | ★ **去重** —— 不能让"一人两机构"翻倍（本指标与机构无关） |
| **总时长** `totalMinutes` | `SUM(duration_min)` **直接对台账** | ★ **只算一次** —— gov **不经机构相加**（若把各机构报表相加会重复） |
| 分项 `byActivityType` | 按 `activity_type` 分组 | 求和恒等于 `totalMinutes`（与 `/me` 同一不变量） |

- **实现已符合**：`buildServiceHours()` 直接查 `volunteer_service_logs`（**不 JOIN `organization_members`**）⇒ 天然只算一次；`participantCount` 已是 `COUNT(DISTINCT user_id)`。
- **验收断言（须有守卫）**：构造"一人同属 A、B 两机构"的夹具 ⇒ ① **gov `totalMinutes` = 该人台账之和（不 ×2）**；② **gov `participantCount` = 1**；③ **A、B 两机构的 `service-hours` 各自都含该人**（(a) 口径的预期重复）。**由 `service-hours-gov.test.ts` 守**（若按机构相加会翻倍 ⇒ 精确变红）。
- ⚠️ **区域维度**：`volunteer_service_logs` **无 `district` 列**（§4.1）⇒ gov 服务时长**不做区县过滤**（与 `people` 同为全局）；按区县出服务时长须先给台账加区域维度（**独立设计**）。

#### 4.5-④ 零时长行的**两端点口径统一**（team-lead 已确认）

**问题**：`started_at_ms == ended_at_ms`（**已确认但零时长**）的行 ⇒ `/me` 返回 `total=0` **且** `breakdown` 有一项（`count=1`）；而 gov 因 `totalMinutes <= 0` 返回 **`null`** ⇒ **同一份数据、两个端点结论相反**。

**统一**：**`null` 判据 = "有无计数中的台账行"，不是"分钟数是否为 0"**（team-lead 倾向，采纳）：

| 端点 | 新口径 |
|---|---|
| `/me` | 维持：有行 ⇒ 返回（`totalMinutes` 可为 0 + `breakdown` 可见） |
| gov `serviceHours` | **改为**：`COUNTING_WHERE` 下**无行** ⇒ `null`；**有行 ⇒ 返回对象**（`totalMinutes` **可为 0**） |

- **理由**：**0 分钟是一个事实（他确实参与过）**，而 **`null` 只表达"无数据"** —— 两者不该混。
- **实现改动**：`buildServiceHours()` 的 `if (totalMinutes <= 0) return null` ⇒ `if (rows.length === 0) return null`（`rows` = 分组结果）。
- **守卫**：新增用例 —— **仅一条零时长行 ⇒ gov 返回 `{totalMinutes:0, participantCount:1, ...}` 而非 `null`**（突变：把判据改回 `totalMinutes<=0` ⇒ 红）。
...
| T16 | **迁移实机**：旧库启动 ⇒ `Migration applied: 041/042/043/044/045/046`（非 skipped）**✅ 已拿到真实日志**（team-lead 记） | 只写 canonical 不写 `migrations[]` | **实机日志断言**（`:memory:` 测不出） | 实机 |
...
| **T41** | ★★ **v1.6 gov 全局口径**：一人同属 A、B 两机构 ⇒ gov `totalMinutes` = 该人台账之和（**不 ×2**）、`participantCount === 1`；**且 A、B 两机构各自的 `service-hours` 都含该人**（(a) 预期重复） | 让 gov 改为"按机构相加" | 「totalMinutes 不翻倍」/「participantCount=1」用例红 | 后端 |
| **T42** | ★★ **v1.6 零时长 null 判据统一**：`COUNTING_WHERE` 下**仅一条零时长行** ⇒ gov 返回**对象**（`totalMinutes:0, participantCount:1`）**而非 `null`**；**无行** ⇒ `null` | 把判据改回 `totalMinutes <= 0` | `null` 判据用例红（两方向） | 后端 |
...
#### 8.3 ⚠️ v1.6 覆盖边界登记（**已知边界，非缺陷**；QA 自承 + team-lead 复核）

| # | 边界 | 说明 / 处置 |
|---|---|---|
| **CB-1** | PII 深扫**只扫键名、未扫"值形态"** | 若把 `phone` 塞进**名字无害**的字段（如 `note`），深扫抓不到 ⇒ **已知覆盖边界**。可选补一条"**扫值形态**"守卫（正则匹配手机号/身份证样态），**成本低、收益一般** ⇒ 记为**可选增强**，不阻塞 |
| **CB-2** | `service:report` 的 `--user` / `--days` 分支未覆盖 | 覆盖边界（非缺陷）；可补 CLI 参数用例 |
| **CB-3** | org 汇总对 `is_drill` / 封顶（480）行的**排除**未单测 | org 复用 `COUNTING_WHERE`（已排除 `is_drill=1`）⇒ 逻辑正确但**未单测**；建议补一条"org 汇总不含 is_drill**行" |
| **CB-4** | org `count` 语义未明确 | `count` = **台账行数**（同 `/me`）；**须在文档/注释写明**避免歧义 |
| **CB-5** | `--days 0` 分支未覆盖 | 覆盖边界；`--days` 应为**正数**（照 `sos-purge` 的用法校验） |

> 以上均为**覆盖边界**，**不是缺陷**；按项目纪律**显式记账**，以免被后人误当"漏报"或误当"缺陷"。可选增强不阻塞交付。
...
| **D-7** | **`volunteer_service_logs.org_id` 从未被写入**（§4.5-②）：`closeServiceForUser()`→`recordService()` **不传 `orgId`** ⇒ 该列**恒为空串** | 机构汇总**只能"按成员"**（(a)），跨机构会重复；"按台账 `org_id`"（(b)）**不可行** | **另立**：① 让写入方传 `org_id`；② **先定义归属规则**（按事件发起方？按人当前机构？）—— **产品决策**，不塞进 F4。**在那之前 (a) 是唯一可行解**（§4.5-①） |
| **D-8** | **`service:report` 的指标口径收窄**（§2.4）：P0 唯一自动来源 = 救援任务 | 用"全部服务参与"作分母会无意义 | 报表**注明分母范围**；其余来源随 P1-10 立项后再放宽 |
...
| **v1.6** | ★ T05 独立验证（NoOne，50 files/470 tests）后 team-lead 复核 —— **澄清三处聚合口径设计空白**，**新增 §4.5**：<br>① **§4.5-①/② 机构归属 = (a) 按成员**（明写"同一人时长会在其每个机构报表各出现一次 = 视图重复非重复计；跨机构相加会重复"）；**根因**＝`volunteer_service_logs.org_id` **从未被写入**（`recordService` 不传 `orgId`）⇒ (b) 按 `org_id` **不可行**、属 P1/另立（**技术债 D-7**）。<br>② **§4.5-③ gov 全局口径**：**人次 `COUNT(DISTINCT user_id)` 去重、总时长对台账只算一次**（不经机构相加）；加**T41** 守"不翻倍"（跨机构夹具）。<br>③ **§4.5-④ 零时长 `null` 判据统一**：**`null` = "有无台账行"**（非"分钟数是否为 0"）—— 0 分钟是事实、`null` 是"无数据"；gov 由 `totalMinutes<=0` 改为 `rows.length===0`；加 **T42**。<br>④ **§8.3 覆盖边界登记 CB-1~CB-5**（PII 深扫只扫键名 / report 参数分支 / org 排除 is_drill 与封顶 / org `count` 语义 / `--days 0`）；**§9 D-7/D-8**；§4.3 验收锚点补 #6/#7 口径条；T16 行更新为 041–046。 |
