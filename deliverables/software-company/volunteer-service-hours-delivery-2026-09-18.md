...
3. **D-7** ✅ **已闭环**（`63dd7c9` + `f069408`）：`resolveUserOrgId` 从 `organization_members` 推导机构归属（admin/manager 优先 → 最早加入 → 无机构 `''`），`closeServiceForUser` 落库 `orgId`，迁移 `047` 回填旧行；机构汇总不再退化为"按成员"口径。`org-caliber-d7.test.ts` + `service-hours-gov.test.ts` 全绿。**原"P1 / 另立"系快照滞后。**
...
2. ~~**定 D-7 优先级**：若政府侧要求机构汇总不重复，需先排 `org_id` 写入（P1/另立）~~ ✅ **D-7 已落地（`63dd7c9`），见 §五.3**
