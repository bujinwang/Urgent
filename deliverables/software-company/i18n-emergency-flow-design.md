...
|---|---|---|---|---|
| **A** | UI 文案 → **必须本地化** | 86 | 20 | **106** |
| **B** | 后端/用户数据透传 → **不得本地化** | 16 | 10 | **26** |
| **C** | 入库 payload → **恒 zh-CN** | 2 | 1 | **3** |
| **D** | 非用户可见（注释/CSS/类名）→ 忽略 | 60 | 43 | **103** |

**这张表就是 P0-4b 的验收口径**：A 类**逐条**必须在 locale 里有对应 key（∅ ≠ 空），
C 类必须**不出现在 page 文件**（已外置），B 类**不得**被造 key。合计 238 条原始命中。
