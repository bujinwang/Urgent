...
> ⚠️ **台账文书缺口（P0-5 发现，已记账、不追溯）**：本表实为 **10 行**，且**编号跳过 `M9`**；
> 而当日 `memory 2026-09-14` 的记录写作「**11 条**突变矩阵」——「11」与表内 10 行对不上，`M9` 既无内容也无结果。
> 属**计数与枚举不一致**，不影响结论（本表 10 行的红/SURVIVED 记录本身是自洽的）。
> 详见 `i18n-mutation-ledger.md` §2 **E-2**。
...
#### 13.1.1 ⚠️ 该边界下**唯一已知的"可见"中文泄漏**（实测确认 → 决定：接受）

上面第 2/3 类不是抽象的 —— 有一个**用户看得见**的出口：
`aed/detail.vue` 的打卡时间线渲染 `ci.comment`（`detail:192`），而该字段的值
**正是本页写入的那条 zh 审计串**（`audit-notes.auditCheckinComment`，如「设备完好，功能正常」）。
⇒ **英文用户查看（尤其他人创建的）打卡记录时，会看到中文备注。**

**决定：接受，按已知边界记账，不在本期修。** 理由：
① 它是**数据**而非文案；② 时间线的**状态徽标**（`ci.status` → `✅ OK` / `⚠️ Has a problem`）
**已经本地化**，中文只出现在**自由文本备注**字段里；③ 真正的解法是**后端按语言返回**或
**数据层多语言**，属 F2 范围外（§2 已声明后端 `Accept-Language` 不做）。

若将来要修，两条路都需要先确认**接口契约**（本项目前端在本页**总是**写入固定串，
但 `comment` 在接口层面是自由字符串，其它客户端理论上可写任意内容）：
(a) 后端 i18n 返回本地化 `comment`；(b) 前端按 `ci.status` 映射 —— 代价是**可能丢弃**非本流程写入的备注。

⚠️ **而且这条泄漏被守卫"夹具"掩盖了 —— 请勿据此错误推论（重要）**：
QA 实测，`aed-detail-drill-i18n.test.ts` 的 **en-US 渲染快照夹具是 `checkIns: []`**
⇒ 快照**根本没走到 `comment` 的渲染路径**。所以：

> **「en 快照绿」≠「页面无中文」。**

这是**守卫夹具的覆盖盲区**，不是守卫失效（守卫本身有效，是该用例的输入够不到这条路径）。
按上面的决策，本期**刻意不给它加守卫** —— 加了会一直红。
若将来后端 i18n 落地，应**同时**补：(1) 夹具里加一条带 `comment` 的打卡记录；(2) 该路径的 en 快照断言。

#### 13.1.2 ⚠️ 刻意不改：`|| s` 的未知码回退（QA 提出，已评估）

`drill/index.vue` 的 `statusLabel` / `scenarioLabel` 在遇到**未知状态码**时回退为**原样显示该码**
（如后端新增 `flood` 场景 ⇒ 中文用户看到 `flood`）。QA 建议改为回退到 `t('common.other')`。
**决定：保留 `|| s`。** 理由：① 这是**既存设计**（P0-4b 之前就是 `|| s`），非本次引入；
② 原码比"其它"更有**可诊断性**（用户/支持能直接报出码值）；③ 触发条件是后端新增枚举值，低频且随发版可察。
⇒ 不为一处低频回退新增 key。

### 13.2 known-defect（既存，**非本次引入，不在 P0-4b 范围**）

**① `drill/index.vue` 的"发起演习"弹窗用原生 HTML 标签**
（`<div>/<label>/<input>/<select>/<h3>`）。uni-app 在**非 H5 端**不支持原生 HTML 标签
⇒ 该弹窗在微信小程序等端**很可能不可用**。本次只做文案本地化，**不修**（属行为/结构改动，需单独立项）。
⚠️ 连带含义：**该弹窗的本地化在小程序端无法被真实验收**（只有 H5 可验）——
下结论时不要把它当成"全端已验证"。

**② `drill/index.vue` 的列表未按页签过滤（功能性缺陷）✅ 已修复（`710f05d`，P0-4b 收尾）**
`v-for="d in drills"`（L10）迭代**全部**演习，而空态判断用 `displayDrills`（L25）
⇒ 切「已完成」页签**不改变列表内容**。既存性已核：`96edf89^` 同形 ⇒ **非本次引入**。

⚠️ **它引出过一个比缺陷本身更隐蔽的问题：新测试把这个错误行为锁进了 CI。**
`aed-detail-drill-i18n.test.ts` 为继续覆盖"卡片文案已本地化"，按**实际行为**断言
`.drill-card` 数量 === 全部演习。实测：把 `drills` 改成 `displayDrills`（即真正修 bug）⇒ **RED×2**
（`expected 2 to be 3`）。**两个都错的东西互相背书** —— 这比普通漏测更隐蔽。

**处置（分两步，均已完成）**：
1. `8919eaf` —— 先在该断言处加**机器可检索的 `KNOWN-BUG` 标记** + 写明"修复时必须同步
   把期望改为 `events.filter(e => e.status === tab)`"，断言消息里也注明"当前为未过滤的全部演习"，
   作为**临时状态**（当时的范围只做文案本地化）。
2. `710f05d` —— 用户拍板"就近修"后**正式修复**：`:10` 改为 `v-for="d in displayDrills"`；
   删掉 `KNOWN-BUG` 注释块；四个期望由 `events…` 改为 `upcoming…`；**新增**一条"切页签**真的**过滤"
   的回归用例（zh/en 各一）。⇒ 代码层 `KNOWN-BUG` 标记已清零（文档侧保留留痕，见 §14.5/§14.8）。

**记一条通用规矩（本次两步走本身就是范例）**：先用 `KNOWN-BUG` 标记把"锁住 bug 的断言"显式记账、
不让它隐身，再在合适的时机修复 —— **标记是过渡态，不是终点**。

> 由此得一条通用规矩：**当测试不得不按"实际（错误）行为"断言时，必须留下可检索的
> `KNOWN-BUG` 标记与同步修复指引** —— 否则测试会从"资产"变成"阻止修复的路障"。
...
## 14. ✅ P0-4b 实现与独立验证记录（2026-09-15）

**范围**：`pages/aed/detail.vue`（670 行，含 §1.3 的 D4 那 1 行）+ `pages/drill/index.vue`（105 行）。
**门禁**：`vue-tsc --noEmit` **0 error**；vitest **46 文件 / 339 用例**（P0-4a 后基线 45/319 ⇒ +1 文件 +20 用例）；
`Errors` 行 **0**；**乱序**（`--sequence.shuffle`）累计 7 个不同种子全部 46/339：
team-lead `88888`/`1234567`/`20260915`/`98765`、QA `5555`/`777777`/`24680`。

**提交**（共 5 个）：
| hash | 内容 |
|---|---|
| `96edf89` | feat：两页文案双语化 + `audit-notes.ts` 外置 + 定时器泄漏修复 |
| `2d6b3b4` | test：新守卫（18 用例）+ 清单扩到 5 页 + 边界断言翻转 |
| `374e09a` | docs：更正 §13.3 清点表（**我抄错了**，见 §14.6） |
| `cf35b0c` | fix：`custodian.roleFallback` 英文精简 |
| `8919eaf` | test：drill 数量断言标 `KNOWN-BUG`（见 §13.2②） |

### 14.1 两路独立验证，突变矩阵**零 SURVIVED**

派了两路**互不重叠**的验证：① 突变矩阵；② 用清点清单做**逐条覆盖率对账**。
① **全部 RED**、`SURVIVED 清单 = 空`：删 en 键（编译期 `TS2719`）、删 zh 键（编译期放行、
运行期 `静态扫描` 咬住）、zh 值置空、drill 退硬编码中文、en 值含中文、插值占位符改坏、
删卸载清理、删 D1 分支、清空 `SCOPE_FILES`。
其中**关键的两条**：
- **删 `onUnmounted` 的 `checkinTimer` 清理 ⇒ 红** ⇒ 证明那条卸载回归用例**不是空转的**。
- **删 D1 的 `getLocale() === 'zh-CN'` ⇒ 红**（`en 应显示本地化兜底: expected '请求过于频繁' to be 'Notification failed'`）
  ⇒ 后端中文没有后门。

> ⚠️ **计数与枚举不一致（P0-5 发现，已记账、不追溯）**：本节与 `backlog §8.6` 早期写作「**10 条**突变」，
> 但正文**只能点名 9 条**；第 10 条当时未留可检索痕迹（`§14.2` 的 D3 回内联攻击与 drill 回归突变的归属无法确证）。
> ⇒ 已把口径改为**不写数字**（"突变矩阵零 SURVIVED"），并交 P0-5 总账登记为文书缺口
> （`i18n-mutation-ledger.md` §2 **E-1**）。**这属于台账文书的疏漏，不是"守卫没做"。**

### 14.2 ★ D3 的"外置"设计被**证明是承重的**（不是形式主义）

QA 专门攻击了这个设计：把 `audit-notes.ts` 的两条中文**重新内联回 `detail.vue`** ⇒
**裸 CJK 守卫立刻红**（命中 `365: '设备完好，功能正常'` 与 `427: notes: '现场取用（责任人联动兜底）'`）。
⇒ "把数据移出被扫描的 page"确实让守卫继续有效，**不存在"可以悄悄把中文塞回页面"的洞**。
（对照：若当初选了"给守卫开白名单"，这次攻击会被白名单放行 —— 这就是否掉白名单的理由。）

### 14.3 ★ 反向漏抽核验：**硬编码英文**（守卫体系的系统性盲区）

裸 CJK 守卫**只抓中文残留** ⇒ 若有人把文案换成**硬编码英文字符串**（而非 key），
**zh-CN 用户会看到英文，而所有守卫都不红**。这是守卫体系**原理上**的缺口，故专门查了：
**未发现"该抽未抽"的硬编码英文**；剩下的全是语言中立项（`m`/`s`、emoji、内部码、颜色值）。
⇒ 当前范围内该盲区**没有实例**，但**它作为一类风险仍然存在**，不能因为"这次没查到"就认为体系已完备。

### 14.4 覆盖率对账：**A 类 107/107 全部对上，漏抽清单为空**

机器核对（HEAD 内容）：两页共 **100 个 distinct key**（detail 67 + drill 33），
zh-CN 与 en-US **双边 0 缺失、0 空值**。另核了四组重复串（`取消` 共用 `common.cancel`；
`⚠️ 有问题`、页签/状态标签各自独立 key；5 个场景名共用 `drill.scenario.*`）、
**`✅ 设备完好` 与 `✅ 完好` 未被合并**、B 类 31 条**无误抽**、C 类确已外置。

### 14.5 ✅ 决策项已拍板并落地：drill 页签过滤缺陷 —— **就近修**（`710f05d`）

见 §13.2②。缺陷本身是 1 行改动（`drills` → `displayDrills`）+ 同步改测试断言，
属**行为改动**、非 i18n 范围。当时列了两条路：
(a) **另立任务**（只留 `KNOWN-BUG` 标记）；(b) **就近修**。
**用户拍板 (b) 就近修**（理由：真实可见的 UI 缺陷、改动极小，且**越晚修，那个锁住 bug 的断言越可能误伤别人**）。
⇒ 落地为 `710f05d`，详见 §14.8。

### 14.6 ⚠️ team-lead 自己犯的一个错误：清点表数字抄错

`c89efd8` 里我把 §13.3 的分类计数表写成 86/20、16/10、2/1、60/43（合计 238），
而 QA 报告原文是 **69/38、20/11、2/0(+1)、37/0（合计 177）** —— **总量与拆分都错**。
成因：我凭印象补了"每文件拆分"，**没有回查原始报告**。
已用 `374e09a` 单独更正并留痕（而不是悄悄改掉），并在 §13.3 让 QA **反向自检**了一遍数字。
⇒ 与本项目反复强调的"**结论碰巧对、依据全错**"是同一类失误，值得记名。

### 14.7 ⚠️ 流程教训：**突变验证是"写"操作，不能与只读验证并行**

我把两路验证**同时**派出去了，但 ① 会改工作区（甚至改测试文件本身）⇒ ② 的目标仓库被污染：
② 连续看到 `M aed/detail.vue`、`M i18n-scope.ts` 与 **3 类瞬时红**，
不得不自行加一组"**干净工作区连跑 4 次 + 以 `HEAD` 内容为准**"的控制实验才排除噪声。
**两路都没错，是我派单方式错了。** 规矩（已入验证手册）：
1. 同一仓库同一时刻**只允许一路做突变**；其余验证串行，或改用 `git show <hash>:<path>` 取内容。
2. 只读验证方须**显式声明**"结论以 `HEAD` 内容为准"，并给出干净窗口的控制实验。
3. 拿到"红"**先看 `git status`**：脏工作区的红先按噪声处理，`cp`+`shasum` 还原后**复跑**再判。

### 14.8 ✅ `710f05d`：drill 列表按页签过滤（既存缺陷修复）+ 独立对抗验证

**改动**（单提交 `710f05d`，父 `8350839`，未 push）：
- `pages/drill/index.vue:10`：`v-for="d in drills"` → `v-for="d in displayDrills"` —— **唯一一行逻辑改动**。
- 测试同步：原「页签 + 演习卡」用例的期望由 `events…` 改为 `upcoming…`（数量 3→2，状态全为 `drill.status.upcoming`），
  `KNOWN-BUG` 注释块删除；**新增**回归用例「切页签**真的**过滤列表」（zh/en 各一）。
- 门禁：`vue-tsc` **0 error**、**46 文件 / 341 用例**（+2）、顺序与两个乱序种子（`1337`/`20260915`）**三次完全一致**、`Errors` 行 **0**。

**独立对抗验证（复核方 ≠ 作者；作者自证不作为结论）**：**4 条突变全部 RED，SURVIVED 清单为空。**

| # | 突变 | 结果 | 关键错误行 |
|---|---|---|---|
| 1 | `:10` 退回 `drills` | RED×4 | `演习卡数量: expected 3 to be 2`、`upcoming 卡数量: expected 3 to be 2` |
| 2 | `:82` 谓词改错（`d.id === tab`） | RED×5 | `upcoming 卡数量: expected +0 to be 2` |
| 3 | `:82` 过滤抽掉（`return drills.value`） | RED×6 | 新用例**第一条**断言 `expected 3 to be 2` ⇒ 咬的是**过滤本身** |
| 4 | ★ completed 页签去掉 `@click`（点击变空操作） | RED×4 | `completed 卡数量: expected 2 to be 1` |

**第 4 条是本轮最有价值的一条** —— 它攻的不是实现，而是**用例自身的可达性**：
若列表是静态的、点击没生效，用例必须失败；实测确实变红 ⇒ 该回归用例**不是"恰好通过"的空转守卫**。
（这正是 §13.1.1 那条"夹具盲区"教训的**主动防御版**：上一轮是泄漏躲在夹具够不到的分支里，这一轮是**主动构造"让用例该红"的突变**去证它不空转。）

**两处复核细节值得记**：
- 新用例里 `.drill-card-title` 的期望是**夹具数据** `d3.title`（ASCII `'Trauma Drill C'`），**不是 locale 文案** ——
  它与模板 `{{d.title}}` 同源，属"渲染值 === 数据源"的自洽检查，**语义正确、无错位**（复核方逐条核过，排除了"和 locale 值比"的误判风险）。
  隐含前提：不同 status 的夹具 `title` 需互异，否则该断言的区分力下降。
- 突变 2/3/4 除新用例外还连带咬红既有用例（「页签+演习卡」、两条空态用例）—— 属**正常连带**（它们同样依赖过滤/点击），非误报。

**验证卫生**：2 文件 `cp` 备份 + `shasum -a256 -c` **2/2 字节一致**；每处突变还原后立刻复跑 `20/20` 绿（不只看变红那次）；`git status` 干净；复核方未 commit、未改文档。

---

## 15. ✅ P1a：语言切换入口（`e58b0f3` + `eca05bf`，2026-09-16）

**范围**：语言切换 UI（「我的」页）+ 原生 tabBar 文案随语言同步 + 「我的」页文案双语化。
**门禁**：`vue-tsc` **0 error**；**48 文件 / 365 用例**（P1a 前 46/341 ⇒ +2 文件 +24 用例）；
`Errors` 行 **0**；乱序 3 种子（`1337`/`2024`/`20260915`）与顺序跑**一致**。`check:vue-singleton` ✓。

### 15.1 关键设计（两条硬约束）

| # | 约束 | 为什么 |
|---|---|---|
| 1 | `utils/tabbar-locale.ts` **不 import `@/i18n`**，翻译函数 `t` 以**参数注入** | `i18n.ts` 的 `setLocale()` 要调它 ⇒ 反向 import 会形成**循环依赖**，ESM 下可能拿到未初始化绑定（TDZ）⇒ **启动即崩**，且堆栈指向无辜模块 |
| 2 | `typeof` 守卫 + **逐项 `try/catch`** | 宿主无 `setTabBarItem` 时静默返回；单条失败不影响其余 tab —— **"语言切了但 app 崩了"远糟于"某个 tab 文案没跟上"** |

**同步的两个时点**（缺一不可）：
① `setLocale()` 末尾（唯一收口点，调用方无法漏同步）；
② `App.vue` 的 `onLaunch` —— **该路径不经过 `setLocale()`**，因为 i18n 是直接以 `resolveInitialLocale()` 建实例的。
`pages.json` 的静态中文**保留不动**（首屏默认值），在调用处已注明原因。

### 15.2 ★ 又一处「语言冻结」，而且是最容易被写成正确样子的那种

`cert/index.vue` 原来的 `tierLabel` 是
`computed(() => ({gold:'金牌', …} as any)[tier] || tier)` —— **看起来在 `computed` 里**，
但映射表是**模块内字面量**，与 `locale` **无依赖关系** ⇒ 切换语言**不会重算**。
修法：改成在 `computed` 内调 `t()`（`t('mine.tier.' + tier)`），使其 subscribe 到 `locale`。

> 与 §11.1 那条"顶层取值会冻结语言"**同源但不同形**：那次是"在 setup 顶层取值"，
> 这次是"**在 computed 里但没引用任何响应式依赖**"。**判据不是"写在哪"，而是"有没有依赖到 `locale`"**。

### 15.3 ⚠️ 新增一条 KPI 适用边界：**语言自名**

`mine.lang.zh` = 「简体中文」在**英文模式下仍然显示中文** —— 这是**刻意的**
（语言选项应始终以该语言自己的名字呈现，否则英文用户不认识「简体中文」）。
⇒ 这是继 `ci.comment`（§13.1.1）之后**第二个"用户可见的中文"实例**，同属 §13.1 的 KPI 适用边界。
处置：en「无中文」快照**先剔除这一条**再判，并在测试文件头部写明理由 —— **避免后人把它当漏抽去"修"**。

### 15.4 突变验证（4 条全 RED、零 SURVIVED，由 team-lead 亲手跑）

| # | 突变 | 结果 |
|---|---|---|
| 1 | `tierLabel` 由 `computed` 改成 setup 期求值（**制造语言冻结**） | RED（冻结回归用例） |
| 2 | 从 `setLocale` 摘掉 `applyTabBarLocale` | RED（`应同步 4 个 tab: expected +0 to be 4`） |
| 3 | 打乱 `TABBAR_ITEMS` 的 index↔键映射 | **RED×4**（含"zh 文案与 `pages.json` 逐字一致且同序"的交叉断言） |
| 4 | 往 cert 页塞回中文字面量 | RED（裸 CJK 守卫，报错直指文件） |

**⚠️ 过程中踩到一个实操坑**：突变 4 第一次跑**是绿的** —— 因为我只跑了 `cert-i18n.test.ts`，
而**裸 CJK 守卫的本体在 `rescue-i18n.test.ts`**（它按同一份 `SCOPE_FILES` 逐文件扫描）。
`cert-i18n.test.ts` 里那条只是断言"SCOPE_FILES 含 cert 页"，**不执行扫描**。
⇒ **跑突变时必须先确认"真正执行该断言的测试文件是哪个"**，否则会把"没跑到"误读成"没咬住"（**假 GREEN**）。

### 15.5 ⚠️ 流程事故：实现方中途 429 中断

`software-engineer` 在写完代码后、提交与验证前，因 **429 额度已用尽**进程终止，
留下 **8 改 + 3 新增、全部未提交且从未跑过门禁**的工作区。
⇒ 由 team-lead 接手：先跑类型检查与全量测试确认可用，再审关键实现，最后**补做 4 条突变**，分两个提交落地。
**教训**：worker 中断后**先查 `git status`** —— 它很可能有可救的产出，但**绝不能默认它是"已验证过的"**
（这次它连一次门禁都没跑过）。

### 15.6 ✅ P1b 已完成（下节 §16，2026-09-16 同一天）

原计划在这里登记 `pages.json` 里 **39 个中文 `navigationBarTitleText`**，但实际动手前先把范围核了一遍，
发现**真正要做的只有 3 个页面** —— 见 §16。

---

## 16. ✅ P1b：原生导航栏标题随语言切换（`48232ab` + `dd39117`）

**门禁**：`vue-tsc` **0 error**；**49 文件 / 374 用例**（P1b 前 48/365 ⇒ +1 文件 +9 用例）；
`Errors` 行 **0**；乱序 3 种子（`1337`/`20260916`/`777`）与顺序跑**一致**。

### 16.1 ★ 先核范围，比原计划小一个数量级

预估"39 个标题"，实际**只需 3 个**：

| 页面 | `navigationBarTitleText` | 是否要同步 | 原因 |
|---|---|---|---|
| `pages/aed/index` | `附近 AED` | ✅ | 已本地化 + **标准**导航栏 |
| `pages/drill/index` | `急救演习` | ✅ | 同上 |
| `pages/cert/index` | `我的徽章` | ✅ | 同上 |
| `pages/rescue/index` | — | ❌ | `navigationStyle:'custom'`（**自绘**导航栏，无原生标题栏） |
| `pages/guide/index` | — | ❌ | 同上 |
| `pages/aed/detail` | — | ❌ | 同上 |
| 其余 ~24 页 | 各种中文 | ❌ | **页面本身尚未本地化** |

⇒ **两条收窄原则，并由测试显式守护（防后人盲目扩张）**：
1. 只为「**文案已本地化**」的页面同步标题 —— 否则会得到「英文标题 + 中文正文」的畸形状态，**比现状更糟**。
2. `navigationStyle:'custom'` 的页面**没有**原生标题栏，同步无从谈起（它们的页内标题本就是 `t()` 渲染的）。

**教训**：`"有 39 处"` 这种数量直觉**必须先在源文件里核一遍**。这正是本会话第三次栽在"凭印象给数字"
（前两次见 §14.6 与 §15.1 的 E-1/E-2）—— 这次好在是**动手前**核的，直接把工作量从 39 降到 3。

### 16.2 ★ 又双一次「语言冻结」——同一根因的第三种形态

`useLocalizedNavTitle` 的第一次实现（写的时候就已经知道这个坑）必须把 `t()` 放在 `watchEffect` **内部**：
```ts
watchEffect(() => { applyNavTitle(i18nGlobal.t(titleKey)) })   // ✅ t 被读取 ⇒ 依赖 locale
// 若写成：const once = t(key); watchEffect(() => applyNavTitle(once))  ❌ 冻结
```
⇒ 至此该**根因已出现三种形态**：① setup 顶层取值（§11.1）② `computed` 内但无 `locale` 依赖（§15.2）
③ `watchEffect` 内但 key 在外部先求值（本节）。
**统一判据：`t()` 的调用点必须在被追踪的作用域内，且直接读到 `locale`。**

### 16.3 ⚠️ 发现并修掉 P1a 遗留的一个守卫缺口

`pages/cert/index.vue` **从未**加入 `i18n.test.ts` 的静态 key 扫描范围 ——
也就是说 P1a 新增的**整页 `mine.*` 键**，此前完全不受「**代码用了但没定义**」这条守卫保护
（`en ⊇ zh` 的类型层守卫只能防"漏译"，防不住"代码里写了不存在的 key ⇒ 显示裸 key"）。
本次已补入 `SCOPE_FILES`。

### 16.4 突变验证（3 条 + 1 组**对照实验**）

| # | 突变 | 结果 |
|---|---|---|
| 1 | `t()` 移到 `watchEffect` 外（制造语言冻结） | RED（`expected 0 to be greater than 0`） |
| 2 | 删掉 cert 页那一行调用 | RED（报错直指文件） |
| 3 | 键名写错 `nav.drill` → `nav.drll` | **RED×2**（静态扫描 + 调用点） |
| 4 | **对照实验**：把新增的正则退回原样、错键仍在 | 「未定义键」守卫**变绿** ⇒ 证明扩展正则是**承重的** |

**第 4 条是这次的关键**，也是从 §9.4 的 A/A′ 对照沿袭下来的手法：
改一版 detectors 之后，**必须同时验"改前该红的不红"**，否则无法证明这次改动真的补上了缺口
（而不是顺便加了一坨没人需要的东西）。结论：**不扩正则的话，nav 键写错会静默上线**。
