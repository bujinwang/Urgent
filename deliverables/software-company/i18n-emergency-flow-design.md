...
`pages.json` 里还有 **39 个中文 `navigationBarTitleText`**（每页导航栏标题）——
需 `uni.setNavigationBarTitle` 逐页同步。体量较大、与 P1a 不耦合，故单列。
另：`cert/` 子页（`interests`/`upload`/`push-settings`）与非急救主链路页面**均不在 F2 范围**。
