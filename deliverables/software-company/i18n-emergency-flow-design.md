...
⚠️ 连带含义：**该弹窗的本地化在小程序端无法被真实验收**（只有 H5 可验）——
这不改变本期范围，但下结论时不要把它当成"全端已验证"。

### 13.3 ✅ 独立清点基线（P0-4b 的覆盖率对账依据）

QA 在实现动手前，从**基线 commit**（`git show a43f87d:<file>`，避开并发编辑）逐条清点了两个文件的
**用户可见字符串**，四分类：

| 分类 | 含义 | detail.vue | drill | 合计 |
|---|---|---|---|---|
| **A** | UI 文案 → **必须本地化** | 69 | 38 | **107** |
| **B** | 后端/用户数据透传 → **不得本地化** | 20 | 11 | **31** |
| **C** | 入库 payload → **恒 zh-CN** | 2 | 0 | **2**（+1 存疑，见 §13.3.4） |
| **D** | 非用户可见（注释/CSS/类名）→ 忽略 | 37 | 0 | **37** |
| | | | | **合计 177** |

**这张表就是 P0-4b 的验收口径**：A 类**逐条**必须在 locale 里有对应 key（∅ ≠ 空），
C 类必须**不出现在 page 文件**（已外置），B 类**不得**被造 key。合计 177 条原始命中。

#### 13.3.1 ⚠️ 清点当天即发现：**残留中文会让裸 CJK 守卫恒红**

即使 A 类全部抽走，仍会残留 `detail.vue:347` / `:404`（C 类）的中文字面量。
⇒ 这正是 D3 选择"**外置到非页面模块**"而不是"给守卫开白名单"的直接原因
（见 D3 行；白名单会削弱守卫，而外置保持守卫纯净）。

#### 13.3.2 4 组**重复串**（"抽一处漏一处 ⇒ 守卫红"高发点，实现时须两处同改）

| 串 | 位置 1 | 位置 2 |
|---|---|---|
| `取消` | `detail:96` 按钮 | `detail:367` `showModal.cancelText` |
| `⚠️ 有问题` | `detail:77` 状态按钮 | `detail:191` 时间线 |
| `即将开始` | `drill:5` tab | `drill:74` `statusLabel` |
| `已完成` | `drill:6` tab | `drill:74` `statusLabel` |
| 5 个场景名 | `drill:46` `<option>` 文本 | `drill:75` `scenarioLabel` |

⚠️ **反向陷阱**：`detail:70` 的 `✅ 设备完好` 与 `:191` 的 `✅ 完好` 是**两个不同的中文串**，
**不可**用同一个 key 合并（合并即改文案）。

#### 13.3.3 量词/单位（**折进整句 key，不许拼装**）

`drill:18` 的 `{{current}}/{{max}}人`、`🎁 +{{n}}分` 含中文量词 ⇒ 按 §3.3「整句一个 key」处理。
语言中立者**不建 key**：`detail:34` 的 `m`、`detail:265` 的 `s`。
（`detail:265` 的 `'已超时'` **要抽** —— 与 `s` 在同一表达式里。）

#### 13.3.4 D5 归属的最终判定（QA 提为"存疑 A/C"，判 **A**）

`drill:64` 的默认值 `深圳湾公园` 会随 `create()` 入库。但它**是可被用户覆盖的自由文本（内容）**，
不是系统写死的审计留痕。D3 要保的是**审计语义与数据一致性**，不是用户填写的内容。
⇒ 归 **A**，抽 key（zh `深圳湾公园` / en `Shenzhen Bay Park`）。
