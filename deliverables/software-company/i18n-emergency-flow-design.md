...
处置（本轮已完成）：在该断言处加**机器可检索的 `KNOWN-BUG` 标记** + 写明"修复时必须同步
把期望改为 `events.filter(e => e.status === tab)`"，断言消息里也注明"当前为未过滤的全部演习"。
**修不修该缺陷本身，已单列为待用户拍板的决策项**（见 §14.5）。
...
### 14.5 ⚠️ 待用户拍板的决策项：drill 页签过滤缺陷**修不修**

见 §13.2②。缺陷本身是 1 行改动（`drills` → `displayDrills`）+ 同步改测试断言，
但属**行为改动**、非 i18n 范围。两条路：
(a) **另立任务**（本轮只留 `KNOWN-BUG` 标记，已做）；
(b) **就近修**（多一个小提交：改 `v-for` + 把断言期望改成按 tab 过滤 + 补一条"切页签确实过滤"的用例）。
**建议 (b)** —— 它是真实可见的 UI 缺陷、改动极小，且**越晚修，那个锁住 bug 的断言越可能误伤别人**。
